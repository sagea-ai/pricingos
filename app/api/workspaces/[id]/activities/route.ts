...
export async function POST(
  ...
  try {
    ...
    const activity = await db.workspaceActivity.create({
      data: {
        type: 'message', // Add required type field
        title: 'New Activity', // Add required title field
        content,
        workspaceId,
        authorId: user.id,
        level,
    ...
    if (mentions.length > 0) {
      await db.workspaceActivityMention.createMany({
        data: mentions.map(userId => ({
          workspaceActivityId: activity.id,
          userId
        }))
      });
      ...
      const notifications = mentionedMembers.map(member => ({
        ...
        message: `${user.firstName || user.email} mentioned you in ${activity.workspace.name}`,
        workspaceId,
      }));
